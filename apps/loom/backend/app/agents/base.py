...
class BaseAgent:
    ...
    async def get_model_client(self, model_override: Optional[str] = None):
        from app.services.model_router import get_best_client  # lazy: added in Phase 2
        preferred = [model_override, *self.definition.model_preference] if model_override else self.definition.model_preference
        return await get_best_client(preferred=preferred)
        ...
        for attempt in range(1, _LLM_RETRIES + 1):
            try:
                async with _LLM_SEMAPHORE:
                    client = await self.get_model_client(model_override)
                    messages = []
                    if system:
