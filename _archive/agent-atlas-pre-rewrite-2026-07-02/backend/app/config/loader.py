...
class ModelConfig:
    ...
    api_key_env: Optional[str] = None  # env var name for cloud provider API keys


@dataclass
class RoutingConfig:
    # Ordered list of model names to probe at runtime — first available wins
...
class ConfigLoader:
    ...
    def _load_models(cls):
        ...
        for name, cfg in data.get("models", {}).items():
            ...
            # Allow per-model endpoint override via env var:
            # e.g. AGENT_ATLAS_OLLAMA_LOCAL_ENDPOINT=http://host.docker.internal:11434/v1/chat/completions
            env_key = f"AGENT_ATLAS_{name.upper()}_ENDPOINT"
            endpoint = os.getenv(env_key, endpoint)

            cls._models[name] = ModelConfig(
                name=name,
    ...
    def _load_agents(cls):
        ...
        for yml_file in sorted(agents_dir.glob("*.yml")):
            try:
                ...
                if "id" in agent_data:
                    cls._agents[agent_data["id"]] = agent_data
    ...
    @classmethod
    def get_routing(cls) -> RoutingConfig:
        return cls._routing
