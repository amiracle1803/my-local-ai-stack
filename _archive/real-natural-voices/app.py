...
# ── Silence third-party noise before any heavy imports ────────
...
try:
    print("  Loading F5-TTS (first run downloads ~1.5 GB model)...")
    import warnings as _w
    with _w.catch_warnings():
        _w.simplefilter("ignore")
        from f5_tts.api import F5TTS
    f5_model = F5TTS(device=device)
    F5_SR    = f5_model.target_sample_rate

    print("  Warming up F5-TTS...")
    _w2, _sr2, _ = f5_model.infer(
        ref_file=F5_DEF_REF, ref_text=F5_DEF_TEXT,
    ...
    if device == "cuda":
        torch.cuda.empty_cache()
    HAS_F5 = True
    print(f"  F5-TTS ready! (sample rate: {F5_SR} Hz)")
except Exception as e:
    print(f"  F5-TTS load failed: {e}")
...
def list_engines():
    return jsonify({"chatterbox": True, "f5": HAS_F5, "kokoro": HAS_KOKORO,
...
def generate():
    ...
    try:
        ...
        if engine == "f5":
            if not HAS_F5:
                return jsonify({"error": "F5-TTS failed to load at startup. Check the console for details."}), 400
            ...
            wav_np, out_sr, _ = f5_model.infer(
                ref_file=ref_wav,
                ref_text=ref_txt,
                gen_text=full_text,
                nfe_step=nfe_steps,
...
def generate_stream():
    ...
    def _run():
        ...
        try:
            ...
            if engine == "f5":
                if not HAS_F5:
                    yield _evt({"type": "error", "error": "F5-TTS failed to load."})
                    return
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    wav_np, sr, _ = f5_model.infer(
                        ref_file=ref_wav, ref_text=ref_txt, gen_text=chunk,
                        nfe_step=nfe_steps, cfg_strength=cfg_strength,
                        speed=1.0, cross_fade_duration=0.0,
                        show_info=lambda *a: None, progress=None,
                    )
                    wav_np = np.array(wav_np, dtype=np.float32)
                    if wav_np.ndim == 1:
            ...
            # ── Chatterbox streaming ──────────────────────────────
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as wm_pool:
                    ...
                    for i, chunk in enumerate(chunks):
                        ...
                        # flush previous watermarked chunk
        ...
        # Save the full concatenated file
        elapsed   = round(time.perf_counter() - start, 3)
...
def humanize():
    ...
    try:
        ...
        # Write Stage 1 to a temp file for Stage 2
        stage1_tmp  = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
        ...
        from pipeline.stage2_xtts import generate as _fish_gen
        _fish_gen(
            text=text,
            reference_audio=stage1_path,
            reference_text=text,
...
def upload_voice():
    ...
    try:
        ...
        # Save at F5_SR (24000 Hz): F5-TTS uses it natively (no quality loss),
        # Chatterbox resamples internally via librosa so it's transparent.
        if sr != F5_SR:
            wav = torchaudio.functional.resample(wav, sr, F5_SR)
        out_path = os.path.join(VOICES_DIR, f"{safe_name}.wav")
        torchaudio.save(out_path, wav.cpu(), F5_SR)
        if ref_text:
...
if __name__ == "__main__":
    ...
    print("\n  Real Natural Voices")
    print("  Chatterbox (Quality) + F5-TTS (Fast)  |  Local  |  Free")
