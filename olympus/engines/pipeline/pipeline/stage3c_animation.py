...
def _available_ram_gb() -> float:
    """Free+reclaimable system RAM in GB (used to avoid kernel OOM kills)."""
    try:
...
def _maybe_restart_comfy_for_ram(comfy: ComfyClient, min_avail_gb: float = 6.0) -> None:
    """Restart ComfyUI between director shots when system RAM is critically low.

    The LTX-2.3 22B model offloads ~12GB to CPU RAM per render (plus the flux
    end-frame model earlier in the shot). On an 8GB-VRAM machine with limited
    RAM the kernel OOM-killer silently kills ComfyUI mid-batch; a clean restart
    fully reclaims that footprint and keeps a long batch alive."""
    avail = _available_ram_gb()
    if avail < min_avail_gb:
        logger.warning("system RAM low (%.1fGB free) - restarting ComfyUI to reclaim LTX offload", avail)
        comfy.restart()


def _take_from_clip_path(clip_path: Path) -> int:
    """Recover the ComfyUI take counter (``..._00004.mp4`` -> 4) from a clip."""
    import re
    m = re.search(r"_(\d{2,5})\.(mp4|mov|webm)$", str(clip_path))
    return int(m.group(1)) if m else 1


def _canonical_clip_path(
    clip_path: Path, project_dir: Path, sid: str, variant: str = "ltx-director",
) -> Path:
    """Rename a rendered clip to the forward-only canonical filename.

    ``clips/sh-001-01_director_00001.mp4`` -> ``clips/{project}_sc001_sh001_cl01_ltx-director_v001.mp4``
    (strips the ComfyUI counter; keeps the machine sid discoverable via the
    ``clip_path`` JSON field). Returns the (possibly renamed) path. Existing
    legacy projects keep their names when the sid has no project prefix."""
    project = identity.project_code(project_dir)
    canonical = identity.canonical_shot_id(sid, project)
    if canonical == sid:
        return clip_path  # legacy project -- keep name
    new = clip_path.parent / identity.artifact_name(
        canonical, "cl01", variant=variant, version=1, ext="mp4")
    if new.exists():
        return new
    clip_path.rename(new)
    return new
...
def _parse_motion_grammar(motion_prompt: str) -> dict[str, str]:
    """Parse a stage3-generated motion prompt into its camera/motion/character
    parts: ``[camera: slow push-in] [motion: leaves, mist] [character: Kana
    turns her head]``. Unknown keys fall back to empty strings."""
    out: dict[str, str] = {}
    for m in re.finditer(r"\[(\w+):\s*([^\]]*)\]", motion_prompt or ""):
...
def _motion_prompt(shot: dict[str, Any], tier: int, template: str = "") -> str:
    """Build the motion prompt for a shot.

    The primary source is the shot's ``motion_prompt`` field generated by stage3
    (the ``[camera] [motion] [character]`` grammar). It carries the per-shot
    character action, environment motion, and camera direction that the LLM
    planned -- feeding it through is what actually animates the character
    instead of leaving the clip static. Falls back to template-specific built-in
    recipes only when the field is absent.
    """
    base = shot.get("sd_prompt", "anime scene")
    comp = shot.get("composition", "medium shot")
    light = shot.get("lighting", "cinematic lighting")

    mp = shot.get("motion_prompt") or ""
    if mp and "[" in mp:
        g = _parse_motion_grammar(mp)
        cam = g.get("camera") or shot.get("camera_movement") or "slow push-in"
        motion = g.get("motion") or "subtle ambient motion"
        char = g.get("character") or "the character"
        if template == _DIRECTOR_TEMPLATE:
            # Director: the text beat drives motion between start/end keyframes.
            # Front-load the character action, then the environment motion, then
            # the camera -- concise so the distilled model acts on it.
            char_phrase = "no character in frame" if char.lower() in ("none", "no one") else char
            return (
                f"{comp}, anime illustration, {light}. {char_phrase}. "
                f"{motion}, {cam}, dynamic anime motion, "
                "stable consistent character and outfit."
            )
        if template == "wan_ti2v.json":
            return (
                f"{base}. {comp}. {light}. {char}. {motion}. Camera {cam}, "
                "stable anime illustration, consistent face and outfit."
            )
        if template == _LTX23_8GB_TEMPLATE:
            return (
                f"{comp}, anime illustration. {char}. {motion}, hair and cloth "
                f"physics, camera {cam}. Stable consistent character and outfit."
            )
        # generic / LTX-2B
        return (
            f"{comp}, anime scene, {light}. {char}. {motion}, camera {cam}, "
            "dynamic anime motion, stable consistent character and outfit."
        )

    # --- fallback recipes (no stage3 motion_prompt available) ---
    ...
    if template == _DIRECTOR_TEMPLATE:
        # LTX Director V2V (permanent engine): the panel is the start keyframe
        # and the text beat drives the motion. Weak "subtle ambient" phrasing
        # left the 97-frame clips near-static (motion gate axis=none + repeat
        # detector flags), so use STRONG explicit motion verbs up front like the
        # proven LTX-23 branch. The keyframe conditions identity; the beat moves
        # the moment.
        cam = shot.get("camera_movement") or "slow push-in"
        if tier == 1:
            return (
                f"{comp}, anime illustration, {light}. The character breathes and "
                "shifts weight, head turning slightly, hair and clothes flowing "
                "with strong wind physics, strands swaying and drifting, fabric "
                "billowing, subtle body motion, camera slowly pushing in, dynamic "
                "anime motion, stable consistent character and outfit."
            )
        return (
            f"{comp}, anime illustration, {light}. One clear character action with "
            f"strong natural motion, hair and clothes flowing dynamically, wind "
            f"physics moving strands and fabric, camera {cam}, dynamic anime motion, "
            "stable consistent character and outfit."
        )
...
def build_director_timeline(
    ...
    Mirrors the reference First & Last Frame technique: the panel is the image
    start keyframe, the shot's motion prompt is the text beat, and (when
    ``end_frame`` is given) a target image pinned to the last frame
    (``isEndFrame``). The end-frame guide gives the model a concrete target to
    animate toward, which prevents the weak-motion / static clips that a
    single start frame + text beat produce. Returns a dict of patch values for
    ...
    if end_frame:
        segments.append(
            {"id": "s_end", "start": frames - 1, "length": 1, "type": "image",
             "imageFile": end_frame, "isEndFrame": True},
        )
        guide_strengths.append("1.0")
    segments.append({"id": "s_tail", "start": frames, "length": 0, "type": "text", "prompt": "", "isEndFrame": False})
    timeline = {
...
def _render_ltx_phase(
    ...
    try:
        ...
        for sid in shot_order:
            ...
            if not block_id:
                continue
            panel = identity.resolve_panel(
                project_dir / "panels" / block_id, sid, identity.project_code(project_dir))
            if not panel.exists():
            ...
            try:
                ...
                clip_path = _canonical_clip_path(clip_path, project_dir, sid, variant="ltx2b")
                ...
                comfy.free()  # release VRAM between shots (8GB fragmentation)
    ...
    try:
        ...
        for sid in shot_order:
            ...
            if cached_clip is not None and cached_clip.exists():
                ...
                d["cache_hit"] = True
                d["approved_version"] = "v001"
                if s := shots_by_id.get(sid):
                    s["motion_tier"] = orig
                    s["clip_path"] = d["clip_path"]
                    s["approved_version"] = d["approved_version"]
                if orig == 1:
                    tier1 += 1
                else:
                    tier2 += 1
            ...
            try:
                ...
                # Reference First & Last Frame technique: generate a target END
                # frame (Flux img2img from the panel) and pin it to the last
                # frame so the model animates toward a concrete destination.
                # Without it, a single start frame + text beat yields weak-motion
                # / static clips (motion gate axis=none + repeat detector flags).
                end_uploaded = None
                try:
                    end_path = _render_end_frame(
                        comfy, project_dir, panel, motion_prompt, sid,
                    ...
                    end_uploaded = comfy.upload_image(end_path, name=f"anim_director_{sid}_end.png")
                except (ComfyError, ContingencyStop):
                    logger.warning("director END frame failed for %s; using start-only timeline", sid)
                    end_uploaded = None

                # Unload the flux end-frame model from RAM/VRAM before the LTX
                # 22B model loads. Both offload weights to system RAM on 8GB;
                # keeping flux resident while LTX loads exhausted RAM and the
                # kernel OOM-killed ComfyUI mid-batch.
                comfy.free()

                timeline = build_director_timeline(uploaded, motion_prompt, global_prompt, frames, end_frame=end_uploaded)
                patch_set = {
                    **timeline,
                    "GLOBAL_PROMPT": global_prompt,
                    "WIDTH": res[0], "HEIGHT": res[1],
                ...
                clip_path = _postprocess_clip(paths[0], skip_ffmpeg=config.animation.ai_upscale)
                clip_path = _canonical_clip_path(clip_path, project_dir, sid, variant="ltx-director")
                ...
                d["approved_version"] = "v001"
                d["take"] = identity.take_name(_take_from_clip_path(clip_path))
                if s := shots_by_id.get(sid):
                    s["motion_tier"] = orig
                    s["clip_path"] = d["clip_path"]
                    s["motion_verified"] = motion
                ...
                consecutive_drift = 0
                _clip_cache_store(clips_dir, key, clip_path)
                # Release VRAM after each director shot (flux end frame + LTX
                # clip). On 8GB the GPU accumulates fragmented VRAM across
                # consecutive generations and hard-segfaults ComfyUI (observed
                # as "Connection refused" mid-run); freeing between shots keeps
                # it stable for the full batch.
                comfy.free()
                _maybe_restart_comfy_for_ram(comfy)
            except ComfyError as exc:
...
def run(
    ...
    *,
    ...
    dropped = [sid for sid in shot_order if sid not in shot_detail]
    if dropped:
        logger.warning(
            "storyboard shot_detail missing %d screenplay shots -- skipping animation: %s",
            len(dropped), dropped,
        )
        shot_order = [sid for sid in shot_order if sid in shot_detail]
